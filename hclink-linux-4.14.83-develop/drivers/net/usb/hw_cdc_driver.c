...
static inline void rx_process (struct hw_cdc_net *dev, struct sk_buff *skb)
{
	if (dev->is_ncm)
   	{   
        ...
        if(skb->len > 0){
            /* 解决skb的truesize太大，导致ip分片包在协议栈组包报错的问题  */
            struct sk_buff 	*skb2 = alloc_skb (skb->len + NET_IP_ALIGN + L2HEADER_RESERVE, GFP_ATOMIC);
        	if (unlikely(!skb2))
        		goto error;
            skb_reserve (skb2, NET_IP_ALIGN + L2HEADER_RESERVE);
...
static int is_ncm_interface(struct usb_interface *intf)
{
    ...
    if(( bif_class == 0x02 && bif_subclass == 0x0d)  
       ...
       ||( bif_class == 0xff && bif_subclass == 0x06 && bif_protocol == 0x16)
...
/*
经过验证，pktflow对数据应该有拷贝。且拷贝了skb的truesize。导致cpu占用过高。
在使用pktflow的时候，先将包拷贝成小包，然后在送入pktflow，提升router性能。
*/
#if 1
        skb2 = alloc_skb (curr_dgram_len + NET_IP_ALIGN + L2HEADER_RESERVE, GFP_ATOMIC);
		if (unlikely(!skb2))
			goto error;
        skb_reserve (skb2, NET_IP_ALIGN + L2HEADER_RESERVE);

        memcpy(skb2->data, (skb->data + curr_dgram_idx), curr_dgram_len);
        skb_put(skb2, curr_dgram_len);
#else
        skb2 = skb_clone(skb, GFP_ATOMIC);
        if (unlikely(!skb2))
			goto error;
...
#define HUAWEI_ECM_ETHERNET_INTF \
    .bInterfaceClass    = 0xFF, \
