...
static int mv3310_read_status(struct phy_device *phydev)
{
	...
	val = phy_read_mmd(phydev, MDIO_MMD_PCS, MV_PCS_BASE_R + MDIO_STAT1);
	printk("marvell 10g phy_read_mmd MDIO_MMD_PCS %d\n", val);
	if (val < 0)
		return val;
	...
	printk("marvell 10g genphy_c45_read_link %d %x\n", val, phydev->mdio.addr);
	if (val < 0)
		return val;
	...
	val = phy_read_mmd(phydev, MDIO_MMD_AN, MDIO_STAT1);
	printk("marvell 10g genphy_c45_read_link MDIO_MMD_AN %d\n", val);
	if (val < 0)
		return val;
	...
	//printk("waiceng phydev->interface = %d\n", phydev->interface);
	if ((phydev->interface == PHY_INTERFACE_MODE_SGMII ||
		 ...
		 */
		//printk("ceng phydev->interface = %d\n", phydev->interface);
...
static struct mdio_device_id __maybe_unused mv3310_tbl[] = {
	...
	{ 0x002b09ab, MARVELL_PHY_ID_MASK },
	{ },
};
