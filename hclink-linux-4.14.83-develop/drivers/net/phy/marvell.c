...
/*add by liji to modify 1512/1543 led at init function*/
...
static int m88e1121_config_aneg_rgmii_delays(struct phy_device *phydev)
{
	...
	/*add by liji to modify rgmii mode
	* 我在改uboot的时候有说明,主要是强制为rgmii_id模式
	*/
...
static int m88e1121_config_init(struct phy_device *phydev)
{
	...
	/*add by liji --给1512初始化的时候加上led的控制，由于我们用到的不是默认的，所以调整下*/
#if 0
	...
	if (err < 0)
		return err;
#endif
	err = phy_write(phydev, MII_88E1121_PHY_LED_CTRL, MII_88E1121_PHY_LED_1512_DEF);	
	if (err < 0)
		return err;
...
static int marvell_read_status_page(struct phy_device *phydev, int page)
{
	...
	/*
	* 这一块做的不好，我暂时还没想出来统一的方式来管理光电切换的时候的
	* 灯的状态。（必须要有状态记录，否则状态未改变时也一样设置LED,引起额外的开销）
	*/
