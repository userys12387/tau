						   ...
						   struct serial_struct __user *retinfo)
{
	...
	tmp.closing_wait = port->port.closing_wait == ASYNC_CLOSING_WAIT_NONE ? ASYNC_CLOSING_WAIT_NONE : port->port.closing_wait / 10;
						   ...
						   struct serial_struct __user *newinfo)
{
	...
	closing_wait = new_serial.closing_wait == ASYNC_CLOSING_WAIT_NONE ? ASYNC_CLOSING_WAIT_NONE : new_serial.closing_wait * 10;
									  ...
									  void (*callback)(struct urb *))
{
	...
	/*
     * 底层负责硬件的部分只管收发，不要求0包。但是Host确认数据接收完成并不是底层的动作，
     * 而是由Host驱动的Class部分决定的。所以没必要通过0包识别
     * 所以在Device端开发的时候，不能把0包的逻辑加到底层，而应该在Class层相对的Function逻辑层
     * 进行判断并发送0包，底层只要支持0数据的发送就可以了。
     * 当然底层驱动自己发0也没错，因为大多数设备收到单独的0跟没收一样
    */
	/* 广和通FM160要求在USB批量输出传输期间添加零包处理机制，所以增加如下改动 -- liji 20220816 */
	if (dir == USB_DIR_OUT)
	{
